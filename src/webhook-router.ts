...
function handleSSE(req: IncomingMessage, res: ServerResponse) {
  ...
  const initData = {
    routes: Object.fromEntries(
      [...routes.entries()].map(([slug, info]) => [slug, info]),
    ),
...
async function handleRegister(req: IncomingMessage, res: ServerResponse) {
  ...
  events.push(event);
  broadcast("route_change", { routes: getRoutesSnapshot() });
...
async function handleWebhook(req: IncomingMessage, res: ServerResponse) {
  ...
  if (token !== SECRET) {
    authSpan.endMs = authSpan.startMs;
    authSpan.durationMs = 0;
  ...
  authSpan.durationMs = Math.round(performance.now() - startTime - authSpan.startMs);
  authSpan.endMs = authSpan.startMs + authSpan.durationMs;
  ...
  try {
    ...
    parseSpan.durationMs = Math.round(performance.now() - startTime - parseSpan.startMs);
    parseSpan.endMs = parseSpan.startMs + parseSpan.durationMs;
    const durationMs = Math.round(performance.now() - startTime);
  ...
  routeSpan.durationMs = Math.round(performance.now() - startTime - routeSpan.startMs);
  routeSpan.endMs = routeSpan.startMs + routeSpan.durationMs;
  ...
  try {
    ...
    fwdSpan.durationMs = Math.round(performance.now() - startTime - fwdSpan.startMs);
    fwdSpan.endMs = fwdSpan.startMs + fwdSpan.durationMs;
    const durationMs = Math.round(performance.now() - startTime);
...
function handleMetrics(_req: IncomingMessage, res: ServerResponse) {
  let text = metrics.formatPrometheus();
  text = text.replace("__ROUTES_COUNT__", String(routes.size));
  res.writeHead(200, { "Content-Type": "text/plain; version=0.0.4; charset=utf-8" });
  res.end(text);
}

function handleDashboard(_req: IncomingMessage, res: ServerResponse) {
  if (!dashboardHtml) {
    res.writeHead(503, { "Content-Type": "text/plain" });
    res.end("Dashboard not available — dashboard.html not found");
    return;
  }
...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    // GET routes
    if (method === "GET") {
      if (url.pathname === "/" || url.pathname === "/dashboard") return handleDashboard(req, res);
      if (url.pathname === "/routes") return handleRoutes(req, res);
      if (url.pathname === "/events/stream") return handleSSE(req, res);
...
server.listen(PORT, "127.0.0.1", () => {
  ...
  log.info("dashboard", { url: `http://127.0.0.1:${actualPort}/` });
